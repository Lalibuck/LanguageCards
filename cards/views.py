...
def make_day_word():  # add translation by different languages
    languages = ("ru", "de", "fr", "es", "it")
    word = random_word()  # random word using random word api
    defin = definition(word)  # definition of the word by ninjas dictionary
    dayword = DayWord.objects.create(word=word, definition=defin)
    dayword_id = dayword.get_index()
    for lang in languages:  # saving to the db
...
def update_word(query, answer):
    ...
    word = query
    if answer:  # checking credibility of the answer
        if word.learning_rate < 7:
            word.learning_rate += 1  # right answer
        word.learning_time = datetime.datetime.now() + rate[word.learning_rate]  # updating the learning time
        word.save()
    else:  # wrong answer
        if word.learning_rate > 1:
            word.learning_rate -= 1  # updating the learning rate
        word.learning_time = datetime.datetime.now() + rate[word.learning_rate]  # updating the learning time
        word.save()


def four_var(query):  # one from four game
    vars = []
    true_word = Words.objects.get(id=query)  # getting the right answer
    vars.append(true_word.mword)
    some_words = Words.objects.exclude(id=query).order_by('?')[:3]  # getting three another random options
    for word in some_words:
        vars.append(word.mword)
    random.shuffle(vars)  # option shuffling
    return vars


def main(request):  # main page
    if DayWord.objects.filter(added_date=datetime.date.today()).exists():  # if the word_of a day exists
        word = DayWord.objects.filter(added_date=datetime.date.today())
    else:  # if the word_of a day not exists
        make_day_word()
        word = DayWord.objects.filter(added_date=datetime.date.today())
    return render(request, 'main.html', {'words': word})


def word(request, date):  # a word of a day page
...
def add(request):  # adding words
    if request.method == "POST":
        ...
        else:  # unseccesfull adding of the learning word
...
def translate(request):  # translation (if necessary) adding words
    if request.method == 'POST':
        mword = words_of_a_day('en', request.POST.get('lword'))  # translation using microsoft translator api
...
@login_required(login_url='/login/')
def learn(request):  # choosing the learning game page
    if request.method == "POST":
...
def one(request):  # one-from-four game
    if request.method == 'POST' and 'check' in request.POST:  # checking the answer
        ...
        query = query[:1]  # choosing apt words
        word = LearningToMeaning.objects.get(words=query)
        revise = Words.objects.get(id=query)
        if request.POST.get('mean').lower() == revise.mword.lower():  # checking correctness of the answer
            update_word(word, True)  # updating the db
            message = 'Correct'
        else:
            update_word(word, False)  # updating the db
        ...
        query = Words.objects.filter(learningtomeaning__learning_lang=request.session['language'],
                                     ...
                                     learningtomeaning__learning_time__lte=datetime.datetime.now(), ).order_by(
            'learningtomeaning__learning_rate', 'learningtomeaning__learning_time')
        query = query[:1]  # choosing the firs one apt word
        if query.exists():  # have words to learn
            vars = four_var(query)  # generating four options
...
def type(request):
    if request.method == 'POST' and 'check' in request.POST:  # checking the answer
        query = Words.objects.filter(learningtomeaning__learning_lang=request.session['language'],
                                     ...
                                     learningtomeaning__learning_time__lte=datetime.datetime.now()).order_by(
            'learningtomeaning__learning_rate', 'learningtomeaning__learning_time')
        query = query[:1]  # getting apt appropriate options(choosing one of them)
        word = LearningToMeaning.objects.get(words=query)
        revise = Words.objects.get(id=query)
        if request.POST.get('mean').lower() == revise.mword.lower():  # checking the correctness of the answer
...
@login_required(login_url='/login/')
def vocab(request):  # generate the list of learning words by language
    if request.method == 'POST':
        ...
        if query.exists():  # checking the existence of the list by opting params
